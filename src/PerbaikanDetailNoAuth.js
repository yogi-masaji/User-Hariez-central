...
function PerbaikanDetailNoAuth() {
    const { id } = useParams();
        ...
        const getUserdetailPerbaikanDetail = async () => {
            try {
                const response = await axios.get(
                    `http://127.0.0.1:3000/detail/statusPerbaikan/${id}`
                );
    ...
    useEffect(() => {
        // Add a check to ensure the element is available
        if (detailPerbaikan) {
...
export default withRoot(PerbaikanDetailNoAuth);
